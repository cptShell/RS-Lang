...
const ResultRound: React.FC<{ result: ListQuestionData[], score: number, maxSeries: number, dataGame: DataGame }> = ({ result, score, maxSeries, dataGame }) => {
  ...
  useEffect(() => {
    const addData = async () => {
      if (userData.message === MESSAGE_IS_AUTH && userData.token){
        ...
        await addStatisticUser(result, userData, dataGame.name, maxSeries);
      }
    }
    addData();
  }, []);
